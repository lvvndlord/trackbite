...
)
{
    ...
    // AKTYWACJA EDYCJI PO DWUKROTNYM KLIKNIĘCIU
    connect(tabela, &QTableWidget::cellDoubleClicked, this, [this, tabela](int row, int column) {
        if (column == 0) { // Tylko po kliknięciu w pierwszą kolumnę (nazwa)
    ...
    for (int i = 0; i < static_cast<int>(produkty.size()); ++i)
    {
        ...
        connect(przyciskUlubione, &QPushButton::clicked, this, [this, nazwa]() {
        ...
        connect(przyciskUsun, &QPushButton::clicked, this, [this, nazwa]() {
            ...
            if (odpowiedz != QMessageBox::Yes) { return; }
...
void MainWindow::on_buttonDodajProdukt_clicked()
{
    ...
    dodajWierszMiary(); // Jeden pusty wiersz na start
    ...
    // Pętla przelatująca przez wszystkie wygenerowane przez użytkownika pola jednostek
...
void MainWindow::otworzOknoEdycji(Produkt p)
{
    ...
    // Wczytanie starych danych makro
    ...
    // Funkcja dodająca wiersz z możliwością wczytania wagi i nazwy
    auto dodajWierszMiary = [&](const std::string& domyslnaNazwa = "sztuka", double domyslnaWaga = 0.0) {
        QHBoxLayout* row = new QHBoxLayout();
    ...
    // --- Wczytywanie istniejących miar dla edytowanego produktu ---
    bool mialInne = false;
    for (const auto& jedn : p.pobierzJednostki()) {
        if (jedn.nazwa != "g" && jedn.nazwa != "gram") { // Pomijamy bazowe "g"
            dodajWierszMiary(jedn.nazwa, jedn.gramyNaJednostke);
            mialInne = true;
        }
    }

    // Jeśli produkt nie miał przypisanej żadnej dodatkowej miary, rysujemy pustą
    if (!mialInne) {
        dodajWierszMiary();
    }
    ...
    // Najpierw bezpiecznie usuwamy ze starej bazy
    bazaProduktow.usunProdukt(p.pobierzNazwe());

    Produkt nowyProdukt(nazwa, { spinKcal.value(), spinBialko.value(), spinWegle.value(), spinTluszcz.value() });

    // Zapamiętujemy czy produkt miał przed edycją gwiazdkę
    ...
    // Jeśli podczas edycji użytkownik wpisał głupoty, cofamy do poprzedniej wersji
    if (!nowyProdukt.czyPoprawny())
    {
    ...
    // Jeśli wszystko poprawne - zatwierdzamy nowy
    bazaProduktow.dodajProdukt(nowyProdukt);
